...
static uint32_t CalcDownlinkFrequency( uint8_t channel )
{
    ...
    //printf(" CalcDownlinkFrequency frequency=%d",frequency);
...
static uint32_t CalcDownlinkChannelAndFrequency( uint32_t devAddr, TimerTime_t beaconTime, TimerTime_t beaconInterval )
{
   ...
   // printf("channel= %d freq= %d",channel,frequency);
...
static void RxBeaconSetup( TimerTime_t rxTime, bool activateDefaultChannel )
{
    ...
    else
    {
        ...
        frequency = CalcDownlinkChannelAndFrequency( 0x2600299D, Ctx.NvmCtx->BeaconCtx.BeaconTime + ( CLASSB_BEACON_INTERVAL / 1000 ),
                                                     CLASSB_BEACON_INTERVAL );

       //ver el calculo con el address device
...
void LoRaMacClassBBeaconTimerEvent( void* context )
{
   ...
   // Ctx.NvmCtx->BeaconCtx.TimeStamp = TimerGetCurrentTime( );
	 SysTime_t current = SysTimeGet();
	 Ctx.NvmCtx->BeaconCtx.TimeStamp= current.Seconds;
...
/*
    ...
    // nextBeacon.Seconds tiene el tiempo del sistema (que le llego del server)

    //ahora nextBeacon.Seconds tiene el tiempo del siguiente beacon
    nextBeacon.Seconds = nextBeacon.Seconds + ( 128 - ( nextBeacon.Seconds % 128 ) );

    Ctx.NvmCtx->BeaconCtx.NextBeaconRx = SysTime2Ms( nextBeacon );
    if( Ctx.NvmCtx->BeaconCtx.NextBeaconRx > CLASSB_BEACON_INTERVAL ) //pregunta si el becon esta dentro de los 128
    {
        Ctx.NvmCtx->BeaconCtx.LastBeaconRx = Ctx.NvmCtx->BeaconCtx.NextBeaconRx - CLASSB_BEACON_INTERVAL; //obtiene la hora en ms que falta para el próximo beacon
    }
    else
    {
    ...
    if( LoRaMacConfirmQueueIsCmdActive( MLME_DEVICE_TIME ) == true )
    {
        ...
        {
            Ctx.NvmCtx->BeaconCtx.Ctrl.BeaconDelaySet = 1;
            Ctx.NvmCtx->BeaconCtx.BeaconTimingDelay = Ctx.NvmCtx->BeaconCtx.NextBeaconRx - currentTimeMs; // obtiene el delay en ms para el proximo beacon

           // printf("delay next becon = %d",Ctx.NvmCtx->BeaconCtx.BeaconTimingDelay);
