...
{
    int8_t dr = rxConfig->Datarate;//DrOffset; Correccion
   ...
   // phyDr = 7U; //DataratesAU915[dr]; //phyDr=SF
    phyDr = DataratesAU915[dr]; //phyDr=SF

    Radio.SetChannel( frequency ); //rxConfig->WindowTimeout

    Radio.SetRxConfig( MODEM_LORA, rxConfig->Bandwidth, phyDr, 1, 0, 8,400, false, 0, false, 0, 0, true, rxConfig->RxContinuous );

   //printf("%d: @@@Configuracion RX; Freq=%d, Bandwidth=%d , DR=%d , WindowsTimeout=%d \n",RtcGetTimerValue(),frequency,rxConfig->Bandwidth,phyDr,rxConfig->WindowTimeout);

   // printf("%d: WindowsTimeout=%d \n",RtcGetTimerValue(),rxConfig->WindowTimeout);
   ...
   // printf("@@@Configuracion RX MaxPayload=%d \n",maxPayload);
...
LoRaMacStatus_t RegionAU915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nbEnabledChannels > 0 )
    {
       ...
       // *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
        ...
        //Canal fijo, 10=917.2MHz
    	//*channel=10;
    	//Programa para variar las frecuencias desde el canal 8=916.8MHz a 15=918.2MHz
    	//k esta definido arriba de todoo
    		*channel = k;
    		k++;
    		if(k==15U){
    			k=8;
